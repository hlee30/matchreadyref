...
def build(base):
    ...
    question_cards = ''.join(
        ...
        f'<div class="saved-actions" hidden><button type="button" class="save-for-later" aria-pressed="false">Save for later ☆</button><span class="saved-status" role="status" aria-live="polite"></span></div>'
    ...
    practice = (
        ...
        '<p>Mixed shows ten questions across all topics. Choosing a topic shows its full set.</p>'
        '<p id="progress-status" role="status" aria-live="polite">Save your completed questions with an account: $4.99 one time.</p>'
        '<div id="progress-upgrade" hidden><p>Unlock saved progress for $4.99, paid once.</p><button type="button" class="button primary" data-start-checkout>Unlock progress ↗</button></div>'
        '<nav id="progress-views" class="progress-views" aria-label="Question progress" hidden>'
        '<a id="practice-remaining" href="/practice-tests.html">Remaining questions</a>'
        '<a id="review-saved" href="/practice-tests.html?saved=1">Saved for later</a>'
        '<a id="review-completed" href="/practice-tests.html?completed=1">Completed questions</a>'
        '</nav></div></aside>'
    ...
    login='''<section class="pagehero wrap narrow"><p class="eyebrow">YOUR MATCH READY REF ACCOUNT</p><h1>Sign in<span class="lime">.</span></h1><p>Create an account to unlock saved question progress for $4.99, paid once. The Quiz remains free and open to everyone.</p></section><section class="wrap narrow auth-layout" aria-label="Account access"><div id="auth-status" role="status" aria-live="polite"></div><div id="auth-forms"><div class="auth-tabs" role="group" aria-label="Choose account action"><button type="button" class="auth-tab" data-auth-mode="signin" aria-pressed="true">Log in</button><button type="button" class="auth-tab" data-auth-mode="signup" aria-pressed="false">Create account</button><button type="button" class="auth-tab" data-auth-mode="reset" aria-pressed="false">Reset password</button></div><form id="auth-form" class="auth-form"><label for="auth-email">Email</label><input id="auth-email" name="email" type="email" autocomplete="email" required><div id="password-field"><label for="auth-password">Password</label><input id="auth-password" name="password" type="password" autocomplete="current-password" minlength="8" required></div><button type="submit" class="button primary" id="auth-submit">Log in ↗</button></form></div><div id="account-panel" hidden><h2>You’re signed in.</h2><p id="account-email"></p><p id="account-progress"></p><div id="account-upgrade" hidden><p>Unlock saved question progress across devices for $4.99, paid once.</p><button type="button" class="button primary" data-start-checkout>Unlock progress ↗</button></div><div id="account-links" hidden><p><a class="text-link" href="/practice-tests.html">Practice remaining questions →</a></p><p><a class="text-link" href="/practice-tests.html?saved=1">Review saved questions →</a></p><p><a class="text-link" href="/practice-tests.html?completed=1">Review completed questions →</a></p></div><button type="button" class="button outline" id="sign-out">Sign out</button></div><form id="new-password-form" class="auth-form" hidden><h2>Choose a new password</h2><label for="new-password">New password</label><input id="new-password" type="password" autocomplete="new-password" minlength="8" required><button type="submit" class="button primary">Save new password ↗</button></form></section>'''
    (ROOT/'login.html').write_text(shell('Log in or create an account','Log in or create a Match Ready Ref account.','/login.html',base,login,'<script type="module" src="/assets/auth.js?v=20260924-paid"></script>').replace('<meta name="robots" content="index,follow,max-snippet:-1">','<meta name="robots" content="noindex,follow">'),encoding='utf-8')
